...
df = pd.read_csv(r"Passwords.txt", sep = ' ', header = None)
...
Answer_1 = sum(list(map(lambda x: 1 if Number_letters(df[2][x], df[1][x]) <= float(df['max'][x]) and 
...
Answer_2 = sum(list(map(lambda x: 1 if match(df, x) == 1 else 0, df.index)))
